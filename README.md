...
```
(ENS. "ВАШ-IP" - это адрес внутренней сетевой карты виртуалки. Карта создается в vagrant файле в секции privete_network.
я указал статический адрес 192.168.56.171)
...
(ENS. под **внешним** здесь имеется в виду адрес сетевой карты виртуалки типа host-only, про него написано чуть выше.
я использую статический адрес 192.168.56.171)
